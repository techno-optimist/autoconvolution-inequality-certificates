...
our knowledge, the first rigorously certified upper bound for the signed
variant. We additionally re-certify, exactly, four published construction
vectors --- Matolcsi--Vinuesa, AlphaEvolve, Boyer--Li, and Jaech--Joseph ---
upgrading two float-only records to theorem grade and correcting two points of
...
autoconvolutions of step-function-approximable $f$ have become standard
...
$0.90156485\ldots$, and the strongest published claim is the float-evaluated
$0.94136$ of Jaech--Joseph \cite{JJ25}, with larger float-only values
(up to $0.9627$) reported by AI-search systems
\cite{MathScale25,SimpleTES26}. For $C_3$, we are not aware of \emph{any}
...
roughly one unit in the last place in each case); what changes is that three
float records become theorems. This note itself was drafted and verified
...
$\int g = (\int f)^2 = h^2 (\sum_i a_i)^2 > 0$, so some $c_m$ is strictly
...
(iv) When $a_i \ge 0$, also $g \ge 0$, and the integral of a nonnegative
piecewise-linear function is the exact trapezoid sum: with zero endpoint
values, $\onenorm{g} = \int g = h \sum_m g(\tau_m) = h^2 \sum_m c_m$, which
equals $(\int f)^2$ by Fubini (an internal consistency check that the
implementation verifies exactly). For $\twonorm{g}^2$, on each knot gap of
...
$[1.2937,\, 1.50286]$ remains wide open.
\end{remark}
...
\[
[\,0.941362894973764322596059,\; 0.941362894973764322596060\,]
\]
...
combined as $c = c_{PP} - 2 c_{PQ} + c_{QQ}$; six random positions were
...
certified nontrivial upper bound for the signed variant. The two classical
...
--- and certifies (now exactly) $C_3 \le 1.4581056857680625\ldots$; it says
...
\bottomrule
\end{tabular}
\caption{The three certified bounds (Theorems~\ref{thm:c1}--\ref{thm:c3}).
...
Theorems~\ref{thm:c1}--\ref{thm:c3}:
\begin{enumerate}
...
All scripts and verifier outputs accompany this note as ancillary files, in
the directories \texttt{scripts/} and \texttt{certs/}. Large input vectors
are regenerable from the cited public sources; hashes are to be frozen per
Section~\ref{sec:pending}, item 4.
...
\item \texttt{certs/recert\_enclosures.txt} --- directed $25$-digit
enclosures for all re-certified values and margins, as quoted in the text.
\end{itemize}

\paragraph{Figure.} \texttt{scripts/make\_figure.py} regenerates
